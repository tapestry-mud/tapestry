...
///   2. Auto-gen gap-fill — generate command help only for resolved commands with no winning
///      hand-authored topic, owned by the resolved command's pack.
...
public sealed class HelpSeal
{
    ...
    public void Seal()
    {
        ...
        // Pass 2: auto-gen gap-fill — only command ids with no winning authored topic.
        var covered = new HashSet<string>(_authoredWinners.Select(w => w.Id), StringComparer.OrdinalIgnoreCase);
        CommandHelpGenerator.GenerateGaps(registrations, covered, _help);
    }
}
